...
```{r cache = T}
breaks <- c(
  min(saeb_mun$mean_grade_exam_wgt, na.rm = T),
  seq(-30, 30, length.out = 6),
  max(saeb_mun$mean_grade_exam_wgt, na.rm = T)
)
...
saeb_mun <- saeb_mun %>% 
  mutate(
    score_breaks = cut(
      ...
      breaks = breaks,
      include.lowest = T,
      labels = breaks[-(length(breaks) - 1)]
    )
  )

breaks_scale <- levels(saeb_mun$score_breaks) %>% 
  as.integer
...
plot_map(
  ...
  breaks = 7,
  fill = "mean_grade_exam_wgt",
  caption = "Source: IBGE, INEP.\nMean test scores weighted by the number of students taking exam."
) +
  ggtitle(
    "SAEB municipal scores: 2007"
  )
```
